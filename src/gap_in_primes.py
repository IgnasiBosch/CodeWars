...
In the example above gap(2, 3, 50) will return [3, 5] or (3, 5) or {3, 5} which is the first pair between 3 and
50 with a 2-gap.
So this function should return the first pair of two prime numbers spaced with a gap of g between the limits m, n if
these numbers exist otherwise nil or null or None or Nothing (depending on the language). In C++ return in such a
case {0, 0}.
...
def gap(g, m, n):
    last_prime = n
    for prime in (i for i in range(m | 1, n + 1, 2) if is_prime(i)):
        if prime - last_prime == g:
            return [last_prime, prime]
        last_prime = prime
